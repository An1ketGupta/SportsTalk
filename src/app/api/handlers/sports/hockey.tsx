...
export default async function HocketMatchesHandler() {
    ...
    {matchData.length === 0 ? (
      <div className="col-span-full text-center py-20">
        <div className="text-gray-400 text-lg font-medium">Loading Hockey games...</div>
        <p className="text-gray-500 text-sm mt-2">Fetching live hockey scores and match data</p>
      </div>
    ) : (
          ...
          >
            ...
            <div className="flex justify-between items-center mb-4">
              ...
              <div className="flex flex-col items-center flex-1">
                ...
                />
                <span className="text-sm font-semibold text-center text-white mb-1">{game.teams.home.name}</span>
                <span className="text-2xl font-bold text-cyan-400">{game.scores.home}</span>
              </div>
              ...
              <div className="flex flex-col items-center flex-1">
                <img
                  ...
                  className="w-14 h-14 object-contain mb-2"
                />
                <span className="text-sm font-semibold text-center text-white mb-1">{game.teams.away.name}</span>
                <span className="text-2xl font-bold text-cyan-400">{game.scores.away}</span>
              </div>
            </div>
            ...
            <div className="bg-gray-800/50 rounded-lg p-3 mb-4">
              <div className="grid grid-cols-2 gap-2 text-xs">
                <div className="flex justify-between">
                  ...
                  <span className="text-white font-medium">{game.periods.first}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">2nd:</span>
                  <span className="text-white font-medium">{game.periods.second}</span>
                </div>
                <div className="flex justify-between">
                  <span className="text-gray-400">3rd:</span>
                  <span className="text-white font-medium">{game.periods.third}</span>
                </div>
                {game.periods.overtime && (
                  <div className="flex justify-between">
                    <span className="text-gray-400">OT:</span>
                    <span className="text-white font-medium">{game.periods.overtime}</span>
                  </div>
                )}
              </div>
              {game.periods.penalties && (
                <div className="flex justify-between mt-2 pt-2 border-t border-gray-700">
                  ...
                  <span className="text-white font-medium text-xs">{game.periods.penalties}</span>
                </div>
              )}
            </div>
            ...
            <div className="space-y-2 text-xs text-gray-400 mb-4">
              <div className="flex items-center gap-2">
                <span>🗓</span>
                <span>{formattedDate} – {formattedTime} ({game.timezone})</span>
              </div>
            </div>
